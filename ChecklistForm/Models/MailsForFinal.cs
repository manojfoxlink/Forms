...
namespace ChecklistForm.Models
{
    public class MailsForFinal
    {
            ...
            public void NotificationEmail(string Mail, string ApproveId)
            {
                ...
                Body = GetEmailContent(Mail, ApproveId);
                Subject = "CIP AUDIT SHEET NG MAIL ALERT";

                SendEmailByService(Mail, name, Body, Subject);
            }
            private string GetEmailContent(string Mail, string ApproveId)
            {
                string Url = ConfigurationManager.AppSettings["ParentUrl"].ToString();

                String Body1 = String.Empty;
                DataTable dt = _checkResult.GetTaskStatus(ApproveId);

                using (StreamReader reader = new StreamReader(System.Web.HttpContext.Current.Server.MapPath("~/EmailTemplates/CIPAuditFinal.html")))
                {
                ...
                string htmlString = " <table style='border-collapse: collapse,width: 100%;'width='100%' border=1 align=center cellpadding=1 cellspacing=3><tr style='background-color:lightgreen' ><th>CreatedDate</th><th>Project</th><th>Line</th><th>Batch</th><th>Zone</th><th>Station</th><th>Activity Description</th><th>Remarks</th><th>Production LineLeader</th><th>Audited By</th></tr>";
                ...
                for (int loopCount = 0; loopCount < dt.Rows.Count; loopCount++)
                {
                    htmlString += "<tr><td>" + Convert.ToDateTime(dt.Rows[loopCount]["CreateDate"]).ToString("yyyy-MM-dd") + "</td><td>"
                                             ...
                                             + dt.Rows[loopCount]["Namee"] + "</td><td>"
                                             + dt.Rows[loopCount]["LineLeader"] + "</td><td>"
                                             + dt.Rows[loopCount]["AuditedBy"] + "</td><td>"
                                             + "</td></tr>";
                }
                htmlString += "</table>";
