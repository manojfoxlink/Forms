...
namespace ChecklistForm.Models
{
    public class Mail
    {
        ...
        public void NotificationEmail(String Managerid, string ApproveName, string Mail, string type, string BodySubject,int ResultId)
        {
            ...
            Body = GetEmailContent(Managerid, ApproveName, Mail, type, ResultId);
                Subject = "Feedback For NG Value";
            
            SendEmailByService(MailTo, name, Body, Subject, type);
        }
        private string GetEmailContent(string name, string PID, string Subject, string type, int ResultId)
        {
            //string Url = ConfigurationManager.AppSettings["ParentUrl"].ToString();

            String Body1 = String.Empty;
            DataTable dt = _objRpt.ParameterAdhesive2(ResultId);

            using (StreamReader reader = new StreamReader(System.Web.HttpContext.Current.Server.MapPath("~/EmailTemplates/NGMail.html")))
            {
            ...
            Body1 = Body1.Replace("{Email}", type);
            string htmlString = " <table style='border-collapse: collapse,width: 100%;'width='100%' border=1 align=center cellpadding=1 cellspacing=3><tr style='background-color:lightgreen' ><th>IssueType</th><th>Project</th><th>Line</th><th>Adhesive Specification</th><th>Status</th><th>Submitted time</th><th>FeedBack</th></tr>";
           // string htmlString = " <table style='border-collapse: collapse,width: 100%;'width='100%' border=0 align=center cellpadding=0 cellspacing=0><tr  ><th>IssueType</th><th>Project</th><th>Line</th><th>Adhesive Specification</th><th>Status</th><th>Submitted time</th><th>FeedBack</th></tr>";
            ...
            {
                htmlString += "<tr><td>" + dt.Rows[loopCount]["Adhesive"] + "</td><td>"
                                         ...
                                         + dt.Rows[loopCount]["CreatedDate"] + "</td><td>"
                                         + dt.Rows[loopCount]["FeedBack"] + "</td><td>"
                                         + "</td></tr>";
            }
            htmlString += "</table>";
        ...
        private void SendEmailByService(string mailto, string name, string body, string Subject, string type)
        {
